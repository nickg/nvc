...
void predef_array_cmp(mir_unit_t *mu, tree_t decl, mir_cmp_t pred)
{
   type_t type = tree_type(tree_port(decl, 0));
   ...
   assert(pred == MIR_CMP_EQ || pred == MIR_CMP_LT || pred == MIR_CMP_LEQ);
   ...
   mir_value_t left_len, right_len;
   left_len = lower_total_elements(mu, ti, lhs_array);
   right_len = lower_total_elements(mu, ti, rhs_array);
   ...
   if (pred == MIR_CMP_EQ) {
      mir_value_t done = mir_build_cmp(mu, MIR_CMP_EQ, i_loaded, left_len);
      ...
      mir_value_t len_ge_r =
         mir_build_cmp(mu, MIR_CMP_GEQ, i_loaded, right_len);
   ...
   if (elem->kind == T_RECORD) {
      ...
      if (pred == MIR_CMP_EQ)
         ...
         mir_build_cond(mu, eq, fail_bb, test_bb);
   }
   else {
      ...
      if (pred == MIR_CMP_EQ) {
         mir_value_t eq = mir_build_cmp(mu, pred, l_val, r_val);
         mir_build_cond(mu, eq, test_bb, fail_bb);
      }
      else {
...
void vhdl_lower_predef(mir_unit_t *mu, object_t *obj)
{
   ...
   case S_ARRAY_NOR:
      ...
      break;
   case S_ARRAY_EQ:
      predef_array_cmp(mu, decl, MIR_CMP_EQ);
      break;
   case S_ARRAY_LT:
