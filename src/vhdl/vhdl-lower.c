                                ...
                                mir_value_t array)
{
   ...
   if (ti->size < SIZE_MAX)
      return mir_const(mu, t_offset, ti->size);

   assert(mir_is(mu, array, MIR_TYPE_UARRAY));

   mir_value_t length = mir_build_uarray_len(mu, array, 0);
   for (int i = 1; i < ti->ndims; i++) {
      mir_value_t dim_len = mir_build_uarray_len(mu, array, i);
