...
static inline unsigned get_next_tag(cover_unit_t *cu)
{
   assert(cu->data == NULL);
   return cu->next_tag++;
}
...
{
   if (s->unit != NULL && s->unit->data != NULL) {
      for (int i = 0; i < s->items.count; i++) {
         cover_item_t *item = &(s->items.items[i]);
         cover_merge_one_item(item, s->unit->data[item->tag]);
      }
   }
...
{
   if (s->unit != NULL && s == s->unit->self) {
      write_u8(CTRL_PUSH_UNIT, f);

      ident_write(s->unit->name, ident_ctx);
      fbuf_put_uint(f, s->unit->next_tag);
   }
   else
...
cover_scope_t *cover_create_unit(cover_data_t *data, ident_t qual,
                                 ...
                                 tree_t unit, ident_t name)
{
   ...
   cover_unit_t *cu = xcalloc(sizeof(cover_unit_t));
   cu->name = qual;

   assert(hash_get(data->units, qual) == NULL);
   hash_put(data->units, qual, cu);
                                       ...
                                       cover_unit_t *unit)
{
   ...
   for (;;) {
      ...
      case CTRL_PUSH_UNIT:
         {
            ...
            cover_unit_t *cu = hash_get(db->units, name);
            if (cu == NULL) {
               cu = xcalloc(sizeof(cover_unit_t));
               cu->name     = name;
               cu->next_tag = fbuf_get_uint(f);
               cu->self = cover_read_scope(db, f, ident_ctx, loc_ctx, cu);

               hash_put(db->units, cu->name, cu);

               APUSH(s->children, cu->self);
            }
            else {
               (void)fbuf_get_uint(f);

               cover_scope_t *child =
                  cover_read_scope(db, f, ident_ctx, loc_ctx, cu);
      ...
      case CTRL_PUSH_SCOPE:
         {
            cover_scope_t *child =
               cover_read_scope(db, f, ident_ctx, loc_ctx, unit);
            APUSH(s->children, child);
         }
         break;
      case CTRL_POP_SCOPE:
...
int32_t *cover_get_counters(cover_data_t *db, ident_t name)
{
   ...
   cover_unit_t *cu = hash_get(db->units, name);
   if (cu == NULL || cu->next_tag == 0)
      return NULL;

   if (cu->data == NULL)
      cu->data = xcalloc_array(cu->next_tag, sizeof(int32_t));
