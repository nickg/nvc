...
cover_scope_t *cover_create_unit(cover_data_t *data, ident_t name,
                                 cover_scope_t *parent, tree_t inst,
                                 tree_t unit)
{
   cover_unit_t *cu = xmalloc(sizeof(cover_unit_t));
   cu->name = name;
   ...
   if (tree_kind(inst) == T_BLOCK) {
      ...
      cu->self = cover_create_scope(data, parent, inst, NULL);
