...
START_TEST(test_seq)
{
   ...
   const error_t expect[] = {
      ...
      { 108, "others choice must appear last" },
      { 115, "case choice must be locally static" },
      { 128, "case choice must be locally static" },
      { 138, "case choice must be locally static" },
      { 141, "missing choice C in case statement" },
      { 147, "missing choice B in case statement" },
      { 151, "invalid use of BIT" },
      { 158, "type mismatch in range" },
      { 162, "case choice range must have type INTEGER" },
      { 166, "right index of case choice range is not locally static" },
      { 176, "type of exit condition must be BOOLEAN" },
      { 191, "cannot associate this expression with parameter" },
      { 199, "type of next condition must be BOOLEAN" },
      { 202, "cannot use next outside loop" },
      { 204, "no nested loop with label FOO" },
      { 217, "duplicate statement label DUP" },
      { -1, NULL }
   };
   expect_errors(expect);
