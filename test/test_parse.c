...
START_TEST(test_array)
{
   ...
   t = tree_type(d);
   fail_unless(type_kind(t) == T_SUBTYPE);
   fail_unless(type_has_constraint(t) == 1);
   r = tree_range(type_constraint(t), 0);
   fail_unless(tree_ival(tree_left(r)) == 1);
   fail_unless(tree_ival(tree_right(r)) == 3);
   fail_unless(tree_has_value(d));
...
START_TEST(test_vhdl2008)
{
   ...
   fail_unless(type_has_constraint(p6t1));
   type_t p6t1e = type_elem(p6t1);
   fail_unless(type_kind(p6t1e) == T_SUBTYPE);
   fail_unless(type_has_constraint(p6t1e));
...
START_TEST(test_subtype2008)
{
   ...
   fail_if(type_has_constraint(sub1));
   ...
   fail_if(type_has_constraint(sub3));
   fail_unless(type_is_unconstrained(sub3));

   type_t sub4 = tree_type(get_decl(p, "SUB4"));
   fail_if(type_has_constraint(sub4));
   fail_unless(type_is_unconstrained(sub4));

   type_t sub5 = tree_type(get_decl(p, "SUB5"));
   fail_unless(type_has_constraint(sub5));
   fail_unless(tree_subkind(type_constraint(sub5)) == C_INDEX);
   ...
   fail_if(type_has_constraint(sub7));
   fail_if(type_is_unconstrained(sub7));

   type_t sub10 = tree_type(get_decl(p, "SUB10"));
   fail_unless(type_has_constraint(sub10));
   fail_unless(type_is_unconstrained(sub10));

   type_t sub11 = tree_type(get_decl(p, "SUB11"));
   fail_unless(type_has_constraint(sub11));
   fail_unless(tree_subkind(type_constraint(sub11)) == C_INDEX);
   fail_unless(type_is_unconstrained(sub11));

   type_t sub13 = tree_type(get_decl(p, "SUB13"));
   fail_if(type_is_unconstrained(sub13));

   type_t sub15 = tree_type(get_decl(p, "SUB15"));
   fail_if(type_is_unconstrained(sub15));
...
START_TEST(test_issue1055)
{
   input_from_file(TESTDIR "/parse/issue1055.vhd");
...
START_TEST(test_issue1124)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1124.vhd");
...
START_TEST(test_issue1174)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1174.vhd");
...
START_TEST(test_vests6)
{
   input_from_file(TESTDIR "/parse/vests6.vhd");

   parse_and_check(T_ENTITY, T_ARCH, T_CONFIGURATION);
...
START_TEST(test_issue1271)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1271.vhd");
...
START_TEST(test_issue1335)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1335.vhd");
...
START_TEST(test_names4)
{
   set_standard(STD_02);

   input_from_file(TESTDIR "/parse/names4.vhd");

   tree_t a = parse_and_check(T_ENTITY, T_ARCH);

   tree_t p = tree_stmt(a, 0);
   ck_assert_tree_kind(p, T_PROCESS);

   tree_t e0 = tree_value(tree_stmt(p, 0));
   ck_assert_tree_kind(e0, T_PROT_FCALL);
   fail_unless(tree_has_type(tree_name(e0)));

   tree_t e1 = tree_value(tree_param(tree_value(tree_stmt(p, 1)), 0));
   ck_assert_tree_kind(e1, T_PROT_FCALL);
   fail_unless(tree_has_type(tree_name(e1)));

   fail_if_errors();
}
END_TEST

START_TEST(test_osvvm8)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/osvvm8.vhd");
...
START_TEST(test_issue1477)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1477.vhd");
...
START_TEST(test_issue1498)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1498.vhd");
...
START_TEST(test_issue1560)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1560.vhd");
...
START_TEST(test_issue1590)
{
   input_from_file(TESTDIR "/parse/issue1590.vhd");
...
START_TEST(test_issue1612)
{
   input_from_file(TESTDIR "/parse/issue1612.vhd");
...
START_TEST(test_issue1614)
{
   set_standard(STD_08);

   input_from_file(TESTDIR "/parse/issue1614.vhd");
